...
import { useMunicipalityStore } from "../stores/municipality";
...
const useMunicipality = () => {
    ...
    const fetchRecords = async (tableOptions = options.value) => {
        ...
        try {
            ...
            alert.error("No fue posible obtener los municipios.");
        } finally {
            loading.value = false;
        }
    };
    ...
    const save = async () => {
        ...
        try {
            const { data } = editedIndex.value === -1
                ? await municipalityApi.post(null, payload)
                : await municipalityApi.put(`/${editedItem.value.id}`, payload);
    ...
    const deleteItemConfirm = async () => {
        ...
        try {
            const { data } = await municipalityApi.delete(`/${editedItem.value.id}`);
            alert.success(data.message);
            closeDelete();
            await fetchRecords();
        } catch (error) {
